...
import { Bootstrap } from '@libp2p/bootstrap'
  ...
  export default (testContext: {
    ...
    describe('DIDComm using did:ethr:ganache flow', () => {
      ...
      const listenerMultiAddrPrefix = `/ip4/0.0.0.0/tcp/10333`
      ...
      beforeAll(async () => {
        ...
        await new Promise(async (resolve) => {
          ...
          listenerNode = await createLibp2p({
            peerId: idListener,
            addresses: {
              listen: [
                listenerMultiAddrPrefix,
              ],
            },
          ...
          await listenerNode.handle('didcomm/v2', async ({ stream }) => {
            console.log("THERE IS SOME STREAM THING HAPPENING")
          })
      ...
      afterAll(async () => {
        try {
          await new Promise((resolve, reject) => listenerNode.stop())
        } catch (e) {
          //nop
        }
