...
import { Credential, createCredential } from './entities/credential'
import { Presentation, createPresentation  } from './entities/presentation'
import {
  Connection,
  Not,
  LessThan,
...
export class DataStoreORM implements IAgentPlugin {
  ...
  private async messagesQuery(args: FindArgs<MessageColumns>, context: IContext): Promise<SelectQueryBuilder<Message>> {
    ...
    if (context.authenticatedDid) {
      qb = qb.andWhere(
        new Brackets(qb => {
          qb.where('message.to = :ident', { ident: context.authenticatedDid }).orWhere('message.from = :ident', {
  ...
  private async credentialsQuery(args: FindArgs<CredentialColumns>, context: IContext): Promise<SelectQueryBuilder<Credential>> {
    const where = createWhereObject(args)
    let qb = (await this.dbConnection)
  ...
  async dataStoreORMGetVerifiableCredentials(args: FindArgs<CredentialColumns>, context: IContext): Promise<IVerifiableCredential[]> {
    const messages = await (await this.credentialsQuery(args, context)).getMany()
    return messages.map(createCredential)
  }

  async dataStoreORMGetVerifiableCredentialsCount(args: FindArgs<CredentialColumns>, context: IContext): Promise<number> {
    return (await this.credentialsQuery(args, context)).getCount()
  }

  // Presentations

  private async presentationsQuery(args: FindArgs<PresentationColumns>, context: IContext): Promise<SelectQueryBuilder<Presentation>> {
    const where = createWhereObject(args)
    let qb = (await this.dbConnection)
  ...
  async dataStoreORMGetVerifiablePresentations(args: FindArgs<PresentationColumns>, context: IContext): Promise<IVerifiablePresentation[]> {
    const messages = await (await this.presentationsQuery(args, context)).getMany()
    return messages.map(createPresentation)
  }

  async dataStoreORMGetVerifiablePresentationsCount(args: FindArgs<PresentationColumns>, context: IContext): Promise<number> {
