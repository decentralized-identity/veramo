...
export class Identity extends BaseEntity {
  ...
  @OneToMany((type) => Presentation, (presentation) => presentation.issuer)
  issuedPresentations: Presentation[]

  @ManyToMany((type) => Presentation, (presentation) => presentation.audience)
   ...
   * @param where
   */
  async getLatestClaimValue(dbConnection: Connection, where: { type: string }): Promise<String | undefined> {
    const claim = await dbConnection.getRepository(Claim).findOne({
      where: {
        ...where,
        subject: this.did,
      },
